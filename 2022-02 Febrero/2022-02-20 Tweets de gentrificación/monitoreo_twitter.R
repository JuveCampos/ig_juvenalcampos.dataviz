...
# Datos: 
termino <- "becsherm"
# termino <- "(colonia roma OR condesa OR gentrificación)"
bd <- rtweet::search_tweets(q = termino,
                      n = 15000,
                      include_rts = FALSE,
                      retryonratelimit = TRUE)

# openxlsx::write.xlsx(bd, "datos/becsherm.xlsx")
# openxlsx::write.xlsx(bd, "datos/roma_condesa.xlsx")
# Datos:
bd_2 <- bd %>% 
  filter(!is_retweet) %>% 
...
# Nubes de palabras: ----
cols <- RColorBrewer::brewer.pal(7, "Blues")[4:7]
...
pal_num <- colorNumeric(palette = wesanderson::wes_palettes$Zissou1,
                        domain = freq_palabras$freq,
                        reverse = T)

# color_wc <- pal_num(freq_palabras$freq)
color_wc <- sample(x = wesanderson::wes_palettes$Zissou1, 
