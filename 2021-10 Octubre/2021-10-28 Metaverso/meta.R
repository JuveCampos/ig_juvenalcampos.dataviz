...
bd <- rtweet::search_tweets(q = "metaverso lang:es",
                      n = 15000)

openxlsx::write.xlsx(bd, "tweets_meta.xlsx")

bd <- readxl::read_xlsx("tweets_meta.xlsx")


cols <- RColorBrewer::brewer.pal(5, "Blues") %>% rev()
...
#                              tamanio = 0.5) {
  ...
  data$freq = sqrt(case_when(data$freq > 2600 ~ 2700,
                             TRUE ~ data$freq))
