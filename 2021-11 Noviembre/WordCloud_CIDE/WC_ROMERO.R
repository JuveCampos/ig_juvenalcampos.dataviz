...
# bd <- rtweet::search_tweets(q = "(#YoDefiendoAlCIDE OR #SomosCIDE)",
#                       n = 15000)
#
# openxlsx::write.xlsx(bd, "tweets_yo_defiendo_al_CIDE.xlsx")

bd <- readxl::read_xlsx("tweets_yo_defiendo_al_CIDE.xlsx")
...
r = bd$text %>%
  str_to_lower() %>%
  str_detect(pattern = "romero")
bd$text[r]

# Nube de palabras:

data = unique(bd$text[r])
stop_words = c()
num_words = 300
background = "white"
mask = "apple.png"
