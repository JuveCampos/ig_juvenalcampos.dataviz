...
bd <- rtweet::search_tweets(n = 15000,
                            q = "(#YoDefiendoAlCIDE OR #SomosCIDE)",
...
# openxlsx::write.xlsx(bd, "tweets_yo_defiendo_al_CIDE_27_11_V3.xlsx")
...
# Guardamos el conteo de palabras:
# word,weight,color,angle,font,repeat,url
data %>%
  as_tibble() %>%
  rename(weight = freq) %>%
  mutate(color = "",
         ...
         `repeat` = "",
         url = "") %>%
  write.csv2("conteo_palabras_cide.csv",
             row.names = F)
# ?write.csv2
# ?write_csv
# ?write.csv
