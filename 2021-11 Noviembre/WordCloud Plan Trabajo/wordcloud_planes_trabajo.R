...
# Datos: ----
rom <- read.csv("Versión legible para computadora/romero_sin_stopwords.txt") %>%
  as_tibble() %>%
  rename(text = rom_1)
# %>%
#   pluck(1)

vid <- read.csv("Versión legible para computadora/vidal_sin_stopwords.txt") %>%
  as_tibble() %>%
  rename(text = vid_1)
...
gen_wordcloud(data = rom$text)
gen_wordcloud(data = vid$text)
