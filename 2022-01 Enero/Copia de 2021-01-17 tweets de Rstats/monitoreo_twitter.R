...
lapply(users_2$profile_image_url,
       function(r){
         r = users_2$profile_image_url[1]
         name <- str_extract(r, pattern = "\\/\\w+\\_normal.jpg") %>% 
           str_remove_all(pattern = "\\/|\\.jpg") %>% 
           na.omit()
         curl::curl_download(r, str_c("imagenes_usuario/", 
                                      name, 
                                      ".jpg"))         
       })
